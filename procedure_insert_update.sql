...
PROCEDURE PRC_INSERT(
	P_S_DOMAIN_CODE                IN VARCHAR2,
	P_S_DOMAIN_GROUP               IN VARCHAR2,
...
IS
  V_ID_BUSINESS_DOMAIN NUMBER;  
BEGIN
  --
  FOR CDATA IN (SELECT * FROM &&OWNER_NAME..TABLE_NAME_03) LOOP
  ...
  BEGIN
    ...
    INTO
      ...
      VALUES
      (
	    &&OWNER_NAME..FNC_FMK_JPA_SEQ_NEXTVAL('SeqName01'),
		P_S_DOMAIN_CODE,
		P_S_DOMAIN_GROUP,
		P_S_ACTIVE,		
        SYSDATE,
        'INITIAL DATA',
		CDATA.ID_SAAS_PARTITION
      ) RETURNING ID_BUSINESS_DOMAIN INTO V_ID_BUSINESS_DOMAIN;
  EXCEPTION
  WHEN DUP_VAL_ON_INDEX THEN	
	...
	WHERE S_DOMAIN_GROUP  = P_S_DOMAIN_GROUP
	AND S_DOMAIN_CODE     = P_S_DOMAIN_CODE
	AND ID_SAAS_PARTITION = CDATA.ID_SAAS_PARTITION
	RETURNING ID_BUSINESS_DOMAIN INTO V_ID_BUSINESS_DOMAIN;	
  END;
  --
  IF V_ID_BUSINESS_DOMAIN IS NOT NULL THEN
	  ...
	  BEGIN
		...
		INTO
		  ...
		  VALUES
		  (
			&&OWNER_NAME..FNC_FMK_JPA_SEQ_NEXTVAL('SeqName02'),
			V_ID_BUSINESS_DOMAIN,
			P_S_DESCRIPTION,
			P_S_LOCALE,
			SYSDATE,
			'INITIAL DATA',
	  ...
	  WHEN DUP_VAL_ON_INDEX THEN
	   ...
	   S_SHORT                  = P_S_SHORT
	   WHERE ID_BUSINESS_DOMAIN = V_ID_BUSINESS_DOMAIN
